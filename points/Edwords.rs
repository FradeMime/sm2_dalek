...
//
// 此文件是 curve25519-dalek 的一部分。
// 版权所有 (c) 2016-2021 isis lovecruft
// 版权所有 (c) 2016-2020 Henry de Valence
// 有关许可信息，请参阅 LICENSE。
//
// 作者：
// - isis agora lovecruft <isis@patternsinthevoid.net>
...
//! Curve25519 的群运算，采用 Edwards 形式。
//!
//! ## 编码和解码
//!
//! 编码通过与 `CompressedEdwardsY` 结构体相互转换来完成。
//! 结构体是 `[u8; 32]` 的类型化包装器。
//!
//! ## 相等性测试
//!
//! `EdwardsPoint` 结构体实现了 [`subtle::ConstantTimeEq`]
//! 特性，用于常量时间相等性检查，以及 Rust 的 `Eq` 特性，
//! 用于可变时间相等性检查。
//!
//! ## 余因子相关函数
//!
//! 曲线上点群 \\(\mathcal E\\)
//! 的阶为 \\(|\mathcal E| = 8\ell \\)，因此其结构为 \\( \mathcal
//! E = \mathcal E\[8\] \times \mathcal E[\ell]\\)。挠率
//! 子群 \\( \mathcal E\[8\] \\) 由八个小阶点组成。
//!从技术上讲，整个 \mathcal E\\ 都是挠率，但我们
//! 只用这个词来指代较小的 \mathcal E\[8\]\\ 部分，而不是
//! 较大的素数阶 \mathcal E[\ell]\\ 部分。
//!
//! 要测试一个点是否在 \mathcal E\[8\] \\ 内，请使用
//! [`EdwardsPoint::is_small_order`]。
//!
//! 要测试一个点是否在 \mathcal E[\ell] \\ 内，请使用
//! [`EdwardsPoint::is_torsion_free`]。
//!
//! 要乘以余因子，请使用 [`EdwardsPoint::mul_by_cofactor`]。
//!
//!为了完全避免处理余因子，请考虑使用 Ristretto。
//!
//! ## 标量
//!
//! 标量由 [`Scalar`] 结构体表示。要构造标量，请参阅
//! [`Scalar::from_canonical_bytes`] 或 [`Scalar::from_bytes_mod_order_wide`]。
//!
//! ## 标量乘法
//!
//! Edwards 点的标量乘法由以下函数提供：
//!
//! * `Scalar` 和 `EdwardsPoint` 之间的 `*` 运算符，
//! 执行常数时间的变基标量乘法；
//!
//! * `Scalar` 和 `EdwardsBasepointTable` 之间的 `*` 运算符，
//! 执行常数时间的定基标量乘法；
//! //! * 实现
//! [`MultiscalarMul`](../traits/trait.MultiscalarMul.html) trait，用于
//! 恒定时间变基多标量乘法；
//!
//! * 实现
//! [`VartimeMultiscalarMul`](../traits/trait.VartimeMultiscalarMul.html)
...
//! ## 实现
//!
//! Edwards 算法使用 Hisil、Wong、Carter 和 Dawson 的“扩展扭曲坐标系”以及
//! 相应的完整公式实现。更多详情，
//! 请参阅内部文档的 [`curve_models` 子模块][curve_models]
//!。
//!
//! ## 有效性检查
//!
//! 没有函数可以检查某个点是否有效。
//! 相反，`EdwardsPoint` 结构体保证保存的是曲线上有效的
//! 点。
//!
//! 我们使用 Rust 的类型系统使无效点
//! 不可表示：`EdwardsPoint` 对象只能通过
//! 成功解压压缩点来创建，或者通过
//! 对其他（有效的）`EdwardsPoint` 进行操作来创建。
//!
//! [curve_models]: https://docs.rs/curve25519-dalek/latest/curve25519-dalek/backend/serial/curve_models/index.html

// 我们允许使用非蛇形命名法的名称，因为射影空间中的坐标
// 传统上用其在仿射空间中对应点的大写字母表示。
//是的，你听到了，rustc，我要拥有我的
//仿射和投影蛋糕，并且把它们都吃掉。
...
// 定义CompressedEdwardsY 数组结构
pub struct CompressedEdwardsY(pub [u8; 32]);

// 常量时间比较防止侧信道攻击（如时序攻击），确保比较操作的执行时间不依赖于输入数据。
// 不用改
