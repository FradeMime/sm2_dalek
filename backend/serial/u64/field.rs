...
extern "C" {
    fn sm2_z256_add(r: *mut u64, a: *const u64, b: *const u64) -> u64;
    fn sm2_z256_sub(r: *mut u64, a: *const u64, b: *const u64) -> u64;
    fn sm2_z256_modp_to_mont(a: *const u64, r: *mut u64);
    fn sm2_z256_modp_from_mont(r: *mut u64, a: *const u64);
    fn sm2_z256_modp_mont_mul(r: *mut u64, a: *const u64, b: *const u64);
}
...
impl<'b> AddAssign<&'b FieldElement51> for FieldElement51 {
    fn add_assign(&mut self, _rhs: &'b FieldElement51) {
        let mut result = FieldElement51([0u64; 4]);
        unsafe {
            sm2_z256_add(
                result.0.as_mut_ptr(),
                self.0.as_ptr(),
...
impl<'b> MulAssign<&'b FieldElement51> for FieldElement51 {
    fn mul_assign(&mut self, _rhs: &'b FieldElement51) {
        let mut self_mont = FieldElement51([0u64; 4]);
        let mut rhs_mont = FieldElement51([0u64; 4]);
        let mut result_mont = FieldElement51([0u64; 4]);

        unsafe {
            sm2_z256_modp_to_mont(self.0.as_ptr(), self_mont.0.as_mut_ptr());
            sm2_z256_modp_to_mont(_rhs.0.as_ptr(), rhs_mont.0.as_mut_ptr());

            sm2_z256_modp_mont_mul(
                result_mont.0.as_mut_ptr(),
                self_mont.0.as_ptr(),
                rhs_mont.0.as_ptr(),
            );

            sm2_z256_modp_from_mont(self.0.as_mut_ptr(), result_mont.0.as_ptr());
        }
    }
}
...
impl<'a, 'b> Mul<&'b FieldElement51> for &'a FieldElement51 {
    ...
    fn mul(self, _rhs: &'b FieldElement51) -> FieldElement51 {
        ...
        unsafe {
            ...
            sm2_z256_modp_from_mont(result.0.as_mut_ptr(), result_mont.0.as_ptr());
        }
